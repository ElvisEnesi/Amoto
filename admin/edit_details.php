...
<body>
    <?php  
        if (isset($_SESSION['edit_details'])) {
            ...
            echo $_SESSION['edit_details'];
            echo "</div>";
        }
        unset($_SESSION['edit_details']);
    ?>
        <section class="form">
        <form action="<?= root_url ?>admin/edit_details_logic.php?id=<?= htmlspecialchars($id, ENT_QUOTES, 'UTF-8') ?>" method="post">
            <h1>Edit Details</h1>
